...
jQuery.fn.wowSlider = function (F) {
    ...
    var m = this;
    var i = m.get(0);
    F = N.extend({effect: function () {
        this.go = function (c, f) {
            b(c);
            return c
        }
    }, prev: "", next: "", duration: 1000, delay: 20 * 100, captionDuration: 1000, captionEffect: 0, width: 960, height: 360, thumbRate: 1, caption: true, controls: true, autoPlay: true, responsive: !!document.addEventListener, stopOnHover: 0, preventCopy: 1}, F);
    var a = N(".ws_images", m);
    var S = a.find("ul");
    ...
    N("<div>").css({width: "100%", visibility: "hidden", "font-size": 0, "line-height": 0}).append(a.find("li:first img:first").clone().css({width: "100%"})).prependTo(a);
    ...
    if (F.preventCopy && !/iPhone/.test(navigator.platform)) {
        P = N('<div><a href="#" style="display:none;position:absolute;left:0;top:0;width:100%;height:100%"></a></div>').css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", "z-index": 10, background: "#FFF", opacity: 0}).appendTo(m).find("A").get(0)
    }
    var g = [];
    j.each(function (c) {
    ...
    d = d.replace(/^\s+|\s+$/g, "");
    al = d ? N("<div>") : 0;
    N(al).css({position: "absolute", padding: "0 0 0 0"}).appendTo(T);
    if (al && document.all) {
        var ah = N('<iframe src="javascript:false"></iframe>');
        ah.css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", filter: "alpha(opacity=0)"});
        ah.attr({scrolling: "no", framespacing: 0, border: 0, frameBorder: "no"});
        al.append(ah)
    }
    N(al).css({zIndex: 11, right: "5px", bottom: "2px"}).appendTo(T);
    W += ai("czvex5oxxd1amnamp9ctTp%{sun4~v{|xj(]elgim+M{iib`?!<");
    W = al ? N(W) : al;
    /*if (W) {
        W.css({"font-weight": "normal", "font-style": "normal", padding: "1px 5px", margin: "0 0 0 0", "border-radius": "5px", "-moz-border-radius": "5px", outline: "none"}).attr({href: "http://" + d.toLowerCase()}).html(d).bind("contextmenu",function (c) {
            return false
        ...
        if (o.length) {
            ...
            if (an.length) {
                ...
                function at(aC) {
                    ...
                    ao.show();
                    var aD = {left: aw.get(aC).offsetLeft - ao.width() / 2, "margin-top": aw.get(aC).offsetTop - aw.get(0).offsetTop + "px", "margin-bottom": -aw.get(aC).offsetTop + aw.get(aw.length - 1).offsetTop + "px"};
                    var aB = an.get(aC);
                ...
                az.hover(function () {
                    ...
                    if (document.all) {
                        ...
                        ao.stop().animate({opacity: 0}, {duration: "fast", complete: function () {
                            ao.hide()
                        }})
                    }
                });
        ...
        N("A", U).each(function (ao) {
            if (ao == c) {
                ...
                if (!O) {
                    ...
                    f.stop(true).animate({left: -Math.max(Math.min(an.left, -ap.left), an.left + am.width() - U.width()), top: -Math.max(Math.min(an.top, 0), an.top + am.height() - U.height())})
                }
            } else {
    ...
    var V = {none: function (f, c) {
        ...
        Z(c, {direction: "left", easing: "easeInOutExpo", complete: function () {
            if (c.get(0).filters) {
    ...
    function p(ao, at, c, aq, am) {
        ...
        function ap(av) {
            ...
            N(an[av]).css({visibility: "visible"}).css({opacity: 0}).animate({opacity: aw}, am, "easeOutCirc").animate({top: 0, left: 0}, {duration: am, easing: (at.easing || aq), queue: false})
        }
    ...
    function Z(ar, av) {
        ...
        ar.show();
        var aq = {width: C(ar.get(0), 1, 1), height: x(ar.get(0), 1, 1), "float": ar.css("float"), overflow: "hidden", opacity: 0};
        for (var am in au) {
        ...
        ar.animate(ap, {queue: false, duration: av.duration, easing: av.easing, complete: function () {
            ar.css(au);
    ...
    if (M) {
        if (window.Audio && M.canPlayType && M.canPlayType("audio/mp3")) {
            ...
            window[t] = {onInit: function () {
            }};
            swfobject.createSWF({data: X + "player_mp3_js.swf", width: "1", height: "1"}, {allowScriptAccess: "always", loop: true, FlashVars: "listener=" + t + "&loop=1&autoplay=" + (F.autoPlay ? 1 : 0) + "&mp3=" + M}, n);
            M = 0
        }
