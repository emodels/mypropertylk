...
class MailLog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('from_email, from_name, to_email, to_name, subject, message, entry_date, user, type', 'required'),
			array('user, type', 'numerical', 'integerOnly'=>true),
			array('from_email, from_name, to_email, to_name', 'length', 'max'=>100),
			array('subject', 'length', 'max'=>200),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, from_email, from_name, to_email, to_name, subject, message, entry_date, user, type', 'safe', 'on'=>'search'),
		);
	}
